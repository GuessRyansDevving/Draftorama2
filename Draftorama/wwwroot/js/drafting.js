...
function Pack(card_list) {
    ...
    //Masters sets - add foil as 15th card in the pack
    var masters_set = 0;
    if (card_list[0].name == "Attended_Knight") {
        masters_set = 1; //MM3
    }
    ...
    //if masters set, have to determine a fifteenth card for the foil
    if (masters_set > 0) {
        var rarity_foil = "";
        //Determine rarity of foil card to add
        var foil_roll = Math.random();
        if (foil_roll <= .6667) {
            rarity_foil = "C";
        } else if (foil_roll <= .9167) {
            rarity_foil = "U";
        } else if (foil_roll <= .9895) {
            rarity_foil = "R";
        } else {
            rarity_foil = "M";
        }

        var number_of_mythics = 0;
        var number_of_rares = 0;
        var number_of_uncommons = 0;
        var number_of_commons = 0;

        //MM2017 MM3
        if (masters_set == 1) {
            number_of_mythics = 15;
            number_of_rares = 53;
            number_of_uncommons = 80;
            number_of_commons = 101;
        }

        var com_masters_roll = Math.floor(Math.random() * number_of_commons);
        var unc_masters_roll = Math.floor(Math.random() * number_of_uncommons);
        var rar_masters_roll = Math.floor(Math.random() * number_of_rares);
        var myt_masters_roll = Math.floor(Math.random() * number_of_mythics);

        //Before adding, take out last card that was automatically added
        this.pack_contents.splice(PACK_SIZE - 1, 1);

        if (masters_set == 1) {
            //For MM3
            if (rarity_foil == "C") {
                this.pack_contents.push(MM3_common[com_masters_roll]);
            } else if (rarity_foil == "U") {
                this.pack_contents.push(MM3_uncommon[unc_masters_roll]);
            } else if (rarity_foil == "R") {
                this.pack_contents.push(MM3_rare[rar_masters_roll]);
            } else if (rarity_foil == "M") {
                this.pack_contents.push(MM3_mythic[myt_masters_roll]);
            }
        }
    }
...
function Draft(s1, s2, s3, n_players) {
    ...
    //15 cards in packs for MM3 and masters sets
    if (s1 == MM3) {
        PACK_SIZE = 15;
    }
...
function deck_text() {
    ...
    //Add the sideboard cards to the same output
    cur_html = cur_html + "<br>";

    //Create an array of sideboard card names
    sideboard_names = [];
    sideboard_length = draft.players[0].collection.length;
    for (var i = 0; i < sideboard_length; i++) {
        sideboard_names.push(draft.players[0].collection[i].name);
    }

    //Get the unique names and counts of each sideboard card
    sideboard_array = sideboard_names;
    var sideboardcounts = {};
    sideboard_array.forEach(function (x) { sideboardcounts[x] = (sideboardcounts[x] || 0) + 1; });
    uniquesideboard_array = uniq(sideboard_array);

    //Write the sideboard deck text to the window
    sideboard_array_length = uniquesideboard_array.length;
    for (var i = 0; i < sideboard_array_length; i++) {
        cur_html = cur_html + sideboardcounts[uniquesideboard_array[i]].toString() + " " +
            uniquesideboard_array[i].replace("_", ' ').replace("_", ' ').replace("_", ' ').replace("_", ' ').replace("_", ' ').replace("_", ' ')
            + "<br>";
    }
...
function Draft_MM3() {
    draft = new Draft(MM3, MM3, MM3, 8);
    Print_collection();
    return;
}

function Sealed_MM3() {
    Sealed(MM3, MM3, MM3, MM3, MM3, MM3);
    return;
}
...
window.onload = function () {
    ...
    if (mode == 'Draft_MM3') {
        Draft_MM3();
    } else if (mode == 'Sealed_MM3') {
        Sealed_MM3();
    } else if (mode == 'Draft_AER') {
        Draft_AER();
